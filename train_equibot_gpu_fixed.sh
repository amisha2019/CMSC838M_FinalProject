...
# Fix for PyTorch3D not compiled with GPU support
# The env var forces PyTorch3D to run KNN operations on CPU
export PYTORCH3D_FORCE_CPU=1
...
echo "PyTorch3D fix: Forcing KNN operations to CPU (PYTORCH3D_FORCE_CPU=1)"
...
# First create a patch for the PyTorch3D compatibility issue
echo "Creating patch for PyTorch3D GPU compatibility issue..."

cat > /tmp/pytorch3d_patch.py << 'EOL'
import os
import torch
from pytorch3d.ops.knn import knn_points

# Backup the original function
original_knn_points = knn_points

# Create a wrapper that forces CPU operation when needed
def knn_points_cpu_fallback(p1, p2, *args, **kwargs):
    if os.environ.get('PYTORCH3D_FORCE_CPU', '0') == '1':
        # Move tensors to CPU, compute KNN, then move results back to original device
        original_device = p1.device
        p1_cpu = p1.cpu()
        p2_cpu = p2.cpu()
        
        # Convert all tensor args to CPU
        cpu_args = []
        for arg in args:
        ...
        # Convert all tensor kwargs to CPU
        cpu_kwargs = {}
        ...
        # Run KNN on CPU
        results = original_knn_points(p1_cpu, p2_cpu, *cpu_args, **cpu_kwargs)
        
        # Move results back to original device
        if len(results) == 3:
            dists, idx, nn = results
            return dists.to(original_device), idx.to(original_device), nn.to(original_device)
        else:
            dists, idx = results
            return dists.to(original_device), idx.to(original_device)
    else:
        # Use the original function
        return original_knn_points(p1, p2, *args, **kwargs)

# Apply the monkey patch
import pytorch3d.ops.knn
pytorch3d.ops.knn.knn_points = knn_points_cpu_fallback
print("Applied PyTorch3D KNN CPU fallback patch")
EOL

# Run training with the appropriate config and parameters
# Using all our fixes for the normalizer and encoder_handle
echo "Starting training at $(date)"
python -c "exec(open('/tmp/pytorch3d_patch.py').read())" && \
